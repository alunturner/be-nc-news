...
describe("APP", () => {
  ...
  describe("/api", () => {
    ...
    describe("/articles", () => {
      describe("GET", () => {
        it("200: responds with object, keys of total_count and articles, value an array of objects, each with keys author, title, article_id, topic, created_at, votes, comment_count", () => {
          return request(app)
            .get("/api/articles")
            .query({ limit: 20 })
            .expect(200)
            .then(({ body }) => {
              expect(body).to.have.all.key("articles", "total_count");
        ...
        it("200: total_count is the total count regardless of the limit used", () => {
          return request(app)
            .get("/api/articles")
            .query({ limit: 2 })
            .expect(200)
        ...
        describe("QUERIES", () => {
          ...
          it("200: accepts a limit query to limit the number of results, defaults to 10", () => {
            return request(app)
              .get("/api/articles")
              .query({ limit: 5 })
              .expect(200)
          ...
          it("200: accepts a p query to offset the results", () => {
            return request(app)
              .get("/api/articles")
              .query({ sort_by: "article_id", order: "asc", limit: 5, p: 2 })
              .expect(200)
              .then(({ body }) => {
                expect(body.articles).to.have.length(5);
